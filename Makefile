...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_dtcd_read_graph_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_dtcd_read_graph-*.tar.gz
	echo Create archive \"pp_cmd_dtcd_read_graph-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_dtcd_read_graph-$(VERSION)-$(BRANCH).tar.gz dtcd_read_graph

clean_pack:
	rm -f pp_cmd_dtcd_read_graph-*.tar.gz


pp_cmd_dtcd_read_graph.tar.gz: build
	cd make_build; tar czf ../pp_cmd_dtcd_read_graph.tar.gz dtcd_read_graph && rm -rf ../make_build
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/dtcd_read_graph
